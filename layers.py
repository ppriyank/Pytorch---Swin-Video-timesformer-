...
def window_partition_joint_space_time(x, window_size):
    B, T, H, W, C = x.shape
...
def window_reverse_joint_space_time(windows, window_size, H, W, T):
    B = int(windows.shape[0] / (H * W / window_size / window_size ))
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if min(self.input_resolution) <= self.window_size:
            ...
            self.shift_size = 0
            self.window_size = compute_hcf(self.input_resolution[0], self.input_resolution[1])
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if kwargs["timesformer_mode"] == "joint_space_time":
            print("------- Using joint_space_time.....-------")
            self.blocks = nn.ModuleList([
                SwinTransformerBlock_joint_space_time(dim=dim, input_resolution=input_resolution,
        ...
        elif kwargs["timesformer_mode"] == "divided_space_time":
            print("------- Using divided_space_time.....-------")
            self.blocks = nn.ModuleList([
                SwinTransformerBlock_divided_space_time(dim=dim, input_resolution=input_resolution,
                                     num_heads=num_heads, window_size=window_size,
                                     shift_size=0 if (i % 2 == 0) else window_size // 2,
                                     mlp_ratio=mlp_ratio,
                                     qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        elif kwargs["timesformer_mode"] == "experimentation":
            print("------- Using experimentation.....-------")
            self.blocks = nn.ModuleList([
                SwinTransformerBlock_experimentation(dim=dim, input_resolution=input_resolution,
                                     num_heads=num_heads, window_size=window_size,
                                     shift_size=0 if (i % 2 == 0) else window_size // 2,
                                     mlp_ratio=mlp_ratio,
                                     qkv_bias=qkv_bias, qk_scale=qk_scale,
            ...
            print("------- Using None (mode) & or Space Only.....-------")
            SwinTransformerBlock_divided_space_time
            self.blocks = nn.ModuleList([
...
class SwinTransformerBlock_joint_space_time(SwinTransformerBlock):
    
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, **kwargs):
        super().__init__(dim=dim, input_resolution=input_resolution, num_heads=num_heads, window_size=window_size, shift_size=shift_size,
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            ...
            shifted_x = x
            # B * T , H_cells , W_cells, C
        
        # partition windows
        shifted_x = shifted_x.view(-1, self.sec_len, H, W, C)
        # x--> B ,  T , H_cells , W_cells, C
        x_windows = window_partition_joint_space_time(shifted_x, self.window_size)  # nW*B, window_size, window_size, C
        # x_windows (B * H_cells // window_size * W_cells // window_size, T, window_size , window_size, C

        x_windows = x_windows.view(-1, self.sec_len *  self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        # x_windows (B * H_cells // window_size * W_cells // window_size, T * window_size * window_size, C
        ...
        shifted_x = window_reverse_joint_space_time(attn_windows, self.window_size, H, W, self.sec_len)  # B H' W' C
...
class SwinTransformerBlock_divided_space_time(SwinTransformerBlock):
    ...
    def forward(self, x):
        ...
        x=  x.view(-1, self.sec_len, H, W, C)
        batch_size = x.shape[0]
        x = x.permute(0,2,3,1,4).reshape(-1, self.sec_len, C)
        x = self.drop_path(self.temporal_attn(self.temporal_norm1(x)))
        x = x.reshape(-1, H, W, self.sec_len, C)
...
class SwinTransformerBlock_experimentation(SwinTransformerBlock_joint_space_time):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, **kwargs):
